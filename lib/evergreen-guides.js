        ...
        blocks: [
          {
            type: 'cards',
            items: [
              { title: 'COGS drift', body: '<strong>Symptom:</strong> Landed cost per unit creeps up 3-8% between production runs. Nobody notices because supplier POs are approved individually.<br/><strong>Root cause:</strong> Material surcharges (resin, paper, aluminum), MOQ changes, freight rate increases, tooling amortization on shorter runs.<br/><strong>Fix:</strong> Monthly landed-cost roll-forward with variance to prior 3 runs and to standard cost. Any variance >2% requires supplier explanation and PO note.' },
              { title: 'Freight (inbound + outbound)', body: '<strong>Symptom:</strong> Freight as % of revenue rising quarter over quarter.<br/><strong>Root cause:</strong> DIM weight on parcel, sub-optimized pallet configuration, dead miles on LTL, mode-selection defaults, no 3PL rate rebid in 18+ months.<br/><strong>Fix:</strong> DIM optimization first (usually 8-20% parcel savings), then 3PL rate rebid (usually 5-15%), then mode-shift analysis by lane.' },
              { title: 'Deductions and chargebacks', body: '<strong>Symptom:</strong> Net wholesale revenue 12-20% below invoice revenue, without a clear breakdown of why.<br/><strong>Root cause:</strong> Retailer OTIF chargebacks (Walmart, Target, Kroger), routing-guide violations, ASN failures, damages, shortages.<br/><strong>Fix:</strong> Disaggregate deductions monthly. Each retailer, each type. Track top 3 offenders and fix the operational root cause — usually 3PL / EDI / packaging.' },
              { title: 'Promotional over-spend', body: '<strong>Symptom:</strong> Trade spend as % of gross exceeds 12-18% and does not correspond to visible velocity lift.<br/><strong>Root cause:</strong> Un-tracked MCB, uncoordinated promo calendars across retailers, coop and MDF paid without proof of performance, ad spend on brand terms cannibalizing organic.<br/><strong>Fix:</strong> Trade-spend calendar with pre-committed budget per account. Promo ROI review 60 days after every promotion. Kill or renegotiate promos that fail to lift velocity 20%+.' },
              { title: 'Spoilage, damage, returns', body: '<strong>Symptom:</strong> Adjustments and returns line on P&L exceeds 3-5% of gross.<br/><strong>Root cause:</strong> Case pack under-durable, palletization sub-optimal, code-date shortening at DC, quality escapes at factory.<br/><strong>Fix:</strong> Damage rate audit by SKU, by lane, by retailer. Case-pack redesign is the most common structural fix.' },
            ],
          },
        ],
      },
    ...
    sections: [
      ...
      },
      {
        ...
        blocks: [
          ...
          {
            type: 'table',
            headers: ['Dimension', 'What to look for', 'Weight'],
            rows: [
              ['Capacity fit', 'Current utilization, ability to double your volume within 6 months, allocated line time by contract or handshake', '15'],
              ['Category experience', 'Has run your specific format (bottle size, closure, formula chemistry, allergen protocols) for at least 2 years', '12'],
              ['Quality systems', 'Documented change control, batch records, QC release, deviation handling, third-party audit history (SQF, BRCGS, GMP)', '15'],
              ['Regulatory / certification', 'Current cert list matching your needs (organic, non-GMO, kosher, halal, gluten-free, FDA registration, cGMP)', '10'],
              ['Financial stability', 'Years in operation, ownership structure, ability to demonstrate solvency, credit references from suppliers', '8'],
              ['Communication under pressure', 'Response time on tough questions, willingness to acknowledge constraints, quality of technical explanation', '10'],
              ['Commercial terms', 'MOQ realism, setup fees, tooling ownership, payment terms (Net 30 vs 50% deposit), price validity period', '10'],
              ['Geographic and freight fit', 'Distance from your DCs, freight lanes, ability to ship LTL vs FTL, ocean vs domestic', '5'],
              ['Reference checks', 'Two-year customer reference willing to describe a real problem and how it was handled', '10'],
              ['Fit with growth', '"What happens when we double" is answered with a specific plan, not a shrug', '5'],
            ],
          },
          {
            type: 'wired',
      ...
      },
      {
        ...
        blocks: [
          {
            type: 'math',
            heading: 'Three quotes for the same 20,000-unit run — worked comparison',
            rows: [
              { label: 'Candidate A: FOB per unit', value: '$4.10' },
              { label: 'Candidate B: FOB per unit', value: '$4.35' },
              { label: 'Candidate C: FOB per unit', value: '$4.50' },
              { label: 'A: Setup + tooling amortized on run', value: '+$0.25' },
              { label: 'B: Setup + tooling amortized on run', value: '+$0.05' },
              { label: 'C: Setup + tooling amortized on run', value: '+$0.00 (waived)' },
              { label: 'A: Freight from plant (mid-Atlantic to SLC + LA DCs)', value: '+$0.42' },
              { label: 'B: Freight from plant (Midwest)', value: '+$0.32' },
              { label: 'C: Freight from plant (West Coast)', value: '+$0.18' },
              { label: 'A: Payment terms (50% deposit / Net 30)', value: '+$0.08 cash cost' },
              { label: 'B: Payment terms (Net 30)', value: '+$0.02 cash cost' },
              { label: 'C: Payment terms (Net 45)', value: '-$0.01 cash benefit' },
              { label: 'A: Landed cost per unit', value: '<strong>$4.85</strong>' },
              { label: 'B: Landed cost per unit', value: '<strong>$4.74</strong>' },
              { label: 'C: Landed cost per unit', value: '<strong>$4.67</strong>' },
            ],
            note: 'On per-unit price alone, A wins. On landed cost, C wins by 4% — driven mostly by freight lane and waived setup. And that comparison assumes all three deliver on plan; if A has a history of missing dates and C has never missed one, the real-world winner is C by more than 4%.',
          },
        ],
      },
      {
        ...
        blocks: [
          {
            type: 'cards',
            items: [
              { title: '1. How much do we expect to sell, by SKU, by channel, by month, for the next 12 months?', body: 'Rolling. Updated monthly at minimum. Split committed (POs in hand) from expected (based on run rate + known promotions) from aspirational.' },
              { title: '2. What is our target inventory position at end of each month?', body: 'By SKU, by DC. Sufficient to cover expected sales + safety stock through the next replenishment cycle.' },
              { title: '3. What production orders do we need to place, and when?', body: 'Back-scheduled from expected demand, minus current inventory, minus in-transit, plus safety stock, respecting production lead time and MOQ.' },
              { title: '4. What working capital do we need, and when?', body: 'Cash out for production and freight vs cash in from receivables. The forecast is the primary input to the cash forecast.' },
              { title: '5. What are the constraints and risks?', body: 'Supplier capacity, ingredient availability, freight windows, retailer PO timing, seasonal peaks. Named, dated, ranked.' },
              { title: '6. What changed since last forecast, and why?', body: 'Every update tracks what moved. The learning is in the delta.' },
            ],
          },
        ],
      },
  ...
  'cpg-broker-selection-playbook': {
    ...
    sections: [
      ...
      },
      {
        ...
        lead: 'Commission is a percentage of net wholesale revenue on shipments the broker generates or supports. Below is what those percentages actually cost against operating math.',
        blocks: [
          {
            type: 'math',
            heading: 'Broker economics — worked example: $2M in wholesale supported by broker',
            rows: [
              { label: 'Gross wholesale revenue', value: '$2,000,000' },
              { label: 'Deductions (MCB, chargebacks, damages ~12%)', value: '−$240,000' },
              { label: 'Net wholesale revenue', value: '$1,760,000' },
              { label: 'Broker commission (4%)', value: '−$70,400' },
              { label: 'Broker retainer ($3,000/month × 12)', value: '−$36,000' },
              { label: 'Total broker cost', value: '$106,400' },
            ...
            note: 'Whether $106K is well-spent depends on incremental revenue, retention, and retailer relationships the broker enabled. Model both the with-broker and without-broker cases before signing.',
          },
        ],
      },
      {
    ...
    sections: [
      ...
      },
      {
        ...
        blocks: [
          {
            type: 'table',
            headers: ['Clause', 'What to negotiate', 'What to walk on'],
            rows: [
              ['IP / formula ownership', 'Explicit assignment of your formula, artwork, brand IP to you. Explicit exclusion of any co-man contribution unless separately contracted.', 'Any clause granting the co-man rights to sell your formula to other customers or to use it after termination.'],
              ['Tooling ownership', 'You own the tooling, even if it lives at the co-man plant. Documented in tooling schedule with cost, serial, current location.', 'Co-man ownership of tooling you funded. Ambiguous tooling language.'],
              ['Capacity commitment', 'Allocated units per month or line hours per month, with escalation notice period and consequences for allocation misses.', '"Reasonable commercial efforts" or no capacity clause at all.'],
              ['Quality and liability', 'Defined quality specification, testing protocol, right of rejection, cure period, recall support obligations, liability cap tied to purchase volume.', 'Liability caps at $50K or $100K when your annual purchase is $2M+; no recall support obligation.'],
              ['Force majeure', 'Defined events, notice requirements, mitigation obligations, termination right if extended.', 'Broad force majeure language that shifts all upside to the co-man and all downside to you.'],
              ['Exit terms', 'Termination for cause and for convenience, notice periods, transition support obligations, inventory buy-back or wind-down.', 'Multi-year exclusivity with no termination for underperformance; punitive termination fees.'],
            ],
          },
        ],
      },
  ...
  'cpg-operations-kpis': {
    ...
    sections: [
      ...
      },
      {
        ...
        lead: 'A pre-launch brand does not need a fill rate metric. A $30M brand cannot manage without one.',
        blocks: [
          {
            type: 'stages',
            items: [
              { title: 'Pre-launch to $2M', range: 'Focus: prove product-market fit', body: 'Sell-through velocity, DTC/Amazon contribution margin, cash runway, damage rate. Weekly cadence. Manual spreadsheet is fine. Establish the discipline of measuring; the sophistication comes later.' },
              { title: '$2M to $10M', range: 'Focus: build repeatable operations', body: 'Add forecast accuracy, fill rate, inventory turns, deductions %. Introduce monthly S&OP. Manual spreadsheet plus a demand planning tool. Establish the KPI owners.' },
              { title: '$10M to $30M', range: 'Focus: manage complexity', body: 'Add OTIF by retailer, chargeback rate by retailer, contribution margin by channel, cash conversion cycle. Weekly ops standup, monthly executive S&OP. ERP or dedicated planning tool required.' },
              { title: '$30M+', range: 'Focus: institutionalize discipline', body: 'All 15 metrics on a live dashboard, refreshed daily where possible. Retailer scorecards. Category-specific KPIs (velocity per doors, SPINS panel share, Amazon TACOS). Full ERP integration; automated data pipelines.' },
            ],
          },
        ],
      },
